...
MUTATIONS = [
    ...
    ("browser", "조작된 공유 링크의 모르는 ID 를 그대로 센다", P,
     "    const kept = onlyKnown(out, known);", "    const kept = out;"),
    # ——— v2: 기록은 책에 (CPO 2026-08-28) ———
    ("fast", "작가의 별이 첫 책이 아니라 나중 책의 시각으로 선다", P,
     "    if (cur === undefined || at < cur) readAt.set(a, at);", "    readAt.set(a, at);"),
    ("fast", "모르는 작품의 기록이 유령 작가를 세운다", P,
     "    const a = authorOf(workId);\n    if (!a) continue;", "    const a = authorOf(workId);"),
    ("fast", "v1(작가 단위) 링크가 다시 빈 공유 성좌를 연다", P,
     "    if (carried > 0 && !Object.keys(kept.read).length && !Object.keys(kept.want).length)\n      return null;",
     "    // v1 links open empty shared skies again"),
    ("browser", "작품 행의 기록 표면이 통째로 사라진다", C,
     "    <span className=\"u-wmark\" data-testid=\"work-marks\">",
     "    <span hidden className=\"u-wmark\" data-testid-off=\"work-marks\">"),
    # ——— R13-f: 조속기 · 조준 여유 · 고개 응답 (문 0 3차) ———
    ("flight", "조속기가 사라진다 — 실은 운동량이 곁에서도 광속", S,
     "          const sp = Math.max(-vmax, Math.min(vmax, this.thrust));",
     "          const sp = this.thrust;"),
    ("flight", "조속기가 최근접 별을 잊는다 — 뜻 없는 추력이 다시 광속", S,
     "      let govD = this.nearD;", "      let govD = Infinity;"),
    ("flight", "고개 응답이 사라진다 — 유령 카드의 회귀", S,
     "        this.headTurnDir = off ? toDir : null;", "        this.headTurnDir = null;"),
    ("flight", "고개 응답이 문턱을 잃는다 — 하늘 클릭마다 고개가 돈다", S,
     "        this.headTurnDir = off ? toDir : null;", "        this.headTurnDir = toDir;"),
    ("flight", "조준 여유가 도구의 손(8px)으로 돌아간다", G,
     "export const AIM_HOLD_PX = 24;", "export const AIM_HOLD_PX = 8;"),
    # ——— R13-g: 모든 별의 몸 · 궤도의 서가 · 카드 다이어트 (문 0 4차) ———
    ("flight", "미준비 별이 다시 항성으로만 남는다 (광구 소멸)", S,
     "          : representationFor(ap, h);",
     "          : isLandable(id) ? representationFor(ap, h) : REP_STAR;"),
    ("flight", "광구가 지각 행세를 한다 (정직 경계 붕괴)", S,
     "          if (isLandable(id)) this.paintCrust(body);\n          else this.paintPhotosphere(body);",
     "          this.paintCrust(body);"),
    ("flight", "궤도의 서가가 사라진다", S,
     "          this.ensureSatellites(body);", "          // satellites removed"),
    ("flight", "위성이 계기에 응답하지 않는다", S,
     "    if (hw !== this.hoverWork) {", "    if (false) {"),
    ("browser", "착륙 표면 위로 이웃의 몸이 떠오른다", S,
     "        this.state.landedId && id !== this.state.landedId\n          ? REP_STAR",
     "        false\n          ? REP_STAR"),
    ("browser", "나무위키 회귀 — 작품 산문이 처음부터 펼쳐진다", C,
     "      <div className=\"u-works__prose\" hidden={openWork !== w.id}>",
     "      <div className=\"u-works__prose\" hidden={false}>"),
    ("browser", "나무위키 회귀 — 관계의 왜가 전부 선다", CSS,
     ".u-card [data-testid=\"orbit-relations\"] li:not([data-active=\"1\"]) .u-rel__why {\n  display: none;\n}",
     ".u-card [data-testid=\"orbit-relations\"] li:not([data-active=\"1\"]) .u-rel__why {\n  display: block;\n}"),
    ("browser", "해설 접힘이 열리지 않는다", C,
     "          onClick={() => setWhyOpen(!whyOpen)}",
     "          onClick={() => setWhyOpen(false)}"),
    # ——— R13-h: 우주론의 집행 — 왕따의 수리 · 해상의 생성 · 케플러 · 천구의 호 ———
    ("flight", "렌즈 정격이 ×34 로 돌아간다 (왕따 재림)", G,
     "export const LENS_MAG = 10;", "export const LENS_MAG = 34;"),
    ("flight", "그리는 배율이 다시 원배율이 된다 (왕따시만의 재림)", S,
     "    const m = Math.max(1, Math.min(LENS_MAG, (LENS_MAG * d) / LENS_DIST));",
     "    const m = LENS_MAG;"),
    ("flight", "행성이 다시 갑툭튀한다 (생성 없는 등장)", S,
     "        : (b.satK ?? 0) + (target - (b.satK ?? 0)) * Math.min(1, (dt / 1000) * 4.2);",
     "        : target;"),
    ("flight", "궤도가 다시 균일 고리가 된다 (케플러 상실)", S,
     "      const R = rec.radius * (2.2 + (n > 1 ? (1.1 * i) / (n - 1) : 0.4));",
     "      const R = rec.radius * 2.7;"),
    ("browser", "관계가 다시 내부를 관통한다 (현의 회귀)", S,
    ...
    ("flight", "관측선 — 드래그 부호가 뒤집힌다 (하늘이 손가락 반대로 간다)", S,
     "    this.turn(dx * perPx, dy * perPx);", "    this.turn(-dx * perPx, dy * perPx);"),
    ...
    ("flight", "주권 — 단계를 다시 주시점 거리로 잰다 (자유 비행에서 상수)", S,
     ...
     'resolved > 0 || dist < 1250 || nearD < NAME_NEAR'),
    ...
    ("flight", "주권 — 키보드를 듣지 않는다 (화살표도 +/- 도 죽는다)", S,
     '    this.host.ownerDocument.addEventListener("keydown", this.onKeyDown);', ""),
    ("flight", "주권 — 하늘 어디서도 고개가 돌지 않는다 (걷기 밖 회전 제거)", S,
     "    if (this.anim) return;\n    this.camera.rotateOnWorldAxis(this.camera.up, yaw);\n    this.camera.rotateX(pitch);",
     "    if (this.anim) return;"),
    # 관측선(R13) — 추력이 "시선 정면"이 아니라 "뜻한 곳"으로 가는 부품들.
    # 각 변이는 문 0 이후 신설된 비행 계약(조준 항법·도착=정지·정렬·감속-목표)
    # 중 정확히 하나 이상에 잡혀야 한다.
    ("flight", "관측선 — 조준을 해소하지 않는다 (추력이 늘 시선 정면)", S,
     "    if (px > 0) this.resolveAim(at);", ""),
    ("flight", "관측선 — 지목 별로 호밍하지 않는다", S,
     "          this.thrustDir = to.normalize();", "          this.thrustDir = null;"),
    ("flight", "관측선 — 기수가 목표로 정렬되지 않는다", S,
     "          if (this.thrust > 0 && !this.state.reducedMotion) {", "          if (false) {"),
    ("flight", "관측선 — 별 앞에 서지 않는다 (STANDOFF 제거, 관통)", S,
     "        if (d <= ARRIVE_STANDOFF && this.thrust > 0) {", "        if (false) {"),
    ("flight", "관측선 — 감속이 목표가 아니라 최근접 별을 읽는다", S,
     "    const i = this.aimId !== null ? this.index.get(this.aimId) : undefined;\n    if (i !== undefined) {",
     "    const i = undefined as number | undefined;\n    if (i !== undefined) {"),
    # 접근의 사다리(R13-b) — 정보가 접근의 응답이기를 그치는 방식들.
    ("flight", "사다리 — 접근 통지가 죽는다 (스트립의 원천 소멸)", S,
     "      this.cb.onApproach(apprId, Math.round(apprD));", ""),
    ("mobile", "사다리 — 최근접 팔이 죽는다 (지목 없는 손끝에 스트립이 없다)", S,
     "      } else if (nearId !== null && nearD < NAME_NEAR) {", "      } else if (false) {"),
    ("flight", "사다리 — 카드 게이트 제거 (스트립이 카드 밑에 눌러앉는다)", U,
     "    if (!approach || landedId || focusId) return null;", "    if (!approach) return null;"),
    ("flight", "사다리 — 한 줄 해설 단이 죽는다", U,
     "    if (d <= APPROACH_WHY) {", "    if (false) {"),
    ("flight", "사다리 — 여는 문장 단이 죽는다", U,
     "      if (entry?.world) {", "      if (false) {"),
    # 몸의 회랑 · 몸이 남는 클릭(R13-c) — 문 0 2차의 처방들을 되무르는 방식.
    ("flight", "관측선 — 클릭이 다시 몸을 납치한다 (원인 채널 무시)", S,
     '    const cause = this.cameraCause ?? "pick";', '    const cause = "summon" as "pick" | "summon" | "immediate";'),
    ("flight", "관측선 — 비행 시간이 다시 거리 무관 고정", S,
     "    return Math.min(3800, Math.round(650 + 340 * Math.log2(Math.max(1.01, hi / lo))));",
     "    return 900;"),
    ("flight", "몸의 회랑 — 서가가 다시 코앞으로 (책 1.9권)", S,
     "    const lat = f.bh * (2.1 + 0.35 * spanK + 0.65 * port);", "    const lat = f.bh * (1.9 + 1.0 * port);"),
    ("flight", "몸의 회랑 — 고개 클램프가 다시 62°", G,
     "export const LOOK_YAW_MAX = 88;", "export const LOOK_YAW_MAX = 62;"),
    ...
    ("flight", "심사③ — 즉시 배치가 시선을 잃는다 (딥링크 빈 하늘)", S,
     "      this.camera.lookAt(target);\n      this.anim = null;", "      this.anim = null;"),
    ("flight", "심사② — 렌즈 배율(계측)이 다시 상수 정격 (코앞의 벽)", S,
     "      const lensMagHere = Math.max(1, Math.min(LENS_MAG, LENS_MAG * (d / LENS_DIST)));",
     "      const lensMagHere = LENS_MAG;"),
    ("flight", "심사② — surface 가 착륙 없이도 선다 (가짜 착륙 칩)", S,
     "    const stage: Stage = this.state.landedId && surfaceId",
     "    const stage: Stage = surfaceId"),
    ("flight", "심사⑥ — 미준비 별의 코앞이 다시 원경", S,
     "      : resolved > 0 || focusDist < 1250 || nearD < NAME_NEAR",
     "      : resolved > 0 || focusDist < 1250"),
    ("flight", "심사④ — 벽이 다시 아무것도 가리지 않는다", S,
     "      if (n.dot(dir) < -0.6) return true;",
     ""),
    ("flight", "심사⑤ — 이름표가 다시 드래그를 삼킨다", CSS,
     ".universe .globe-label.is-interactive {\n  pointer-events: none;\n}", ""),
    ("flight", "심사⑤ 후속 — 라벨 상자 픽이 죽는다 (당김이 빗나간다)", S,
     "        if (hit.kind === \"work\" && Math.abs(px - hit.x) <= hit.half && py >= hit.y - 4 && py <= hit.y + hit.h) {",
     "        if (false) {"),
    ("mobile", "심사⑤ 후속 — 이름표 상자 픽이 죽는다 (손끝 첫 탭이 빗나간다)", S,
     "      if (hit.kind === \"author\" && Math.abs(px - hit.x) <= hit.half && py >= hit.y - 4 && py <= hit.y + hit.h) {",
     "      if (false) {"),
    # 관측창 프레임(R13-d) — 배가 창의 가장자리로 존재하기를 그치는 방식.
    ("flight", "관측창 — 프레임이 아예 서지 않는다", U,
     '      {stage !== "surface" ? (', '      {false ? ('),
    ("flight", "관측창 — 표면 게이트 제거 (걷는 몸 위에 창이 남는다)", U,
     '      {stage !== "surface" ? (', '      {true ? ('),
    ("flight", "관측선 — 떠남의 자가 다시 렌즈 거리뿐 (먼 선택이 첫 휠에 죽는다)", S,
     "      focusDist > Math.max(LENS_DIST * 1.6, this.focusDistAtSelect * 1.35)",
     "      focusDist > LENS_DIST * 1.6"),
    ("flight", "관측선 — 제자리 커서가 지목을 잃는다 (락 유지 가드 제거)", S,
