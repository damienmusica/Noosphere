...
// ——— 관측선 (R13) ———
// 카메라는 수학적 구성물이 아니라 위치·방향·속도를 가진 세계 속 물체다(문 0,
// CPO 2026-08-28). 같은 드래그에 법이 셋이던 것(하늘 3.6배·회랑 1.6배·궤도
// 부호 반대)을 고개의 법 하나로 통일하고, 추력은 시선 정면이 아니라 뜻한
// 곳으로 간다.

/** 고개의 법 하나 — 모든 상태에서 드래그 각 = px · (fov/화면높이) · 이 값.
 *  1.9: 한 화면 높이의 드래그가 시야의 약 두 배(fov 42° → 80°)를 돈다.
 *  이전 하늘(OrbitControls 0.42 = 실효 3.6)의 절반 — 문 0 "너무 휙휙"의
 *  직접 처방이고, 회랑(1.6)과의 통일값이다. */
export const TURN_GAIN = 1.9;
/** 지목한 별 앞에 서는 거리. NAME_NEAR(430) 안쪽 — 도착하면 이름이 이미 떠
 *  있고, 원반이 화면에 선 자리다. 스쳐 지나가는 것은 도착이 아니다. */
export const ARRIVE_STANDOFF = 140;
/** 비행 중 기수가 지목한 별 쪽으로 정렬되는 최대 각속도(라디안/초).
 *  손을 대지 않아도 목표가 화면 중앙으로 걸어 들어오는 속도 — 낚아채지 않고
 *  데려간다. */
export const ALIGN_RATE = 1.1;

// ——— 조속기와 고개 응답 (R13-f) ———
// 문 0 3차(CPO 2026-08-29): "행성 가까워졌는데도 광속으로 스크롤되니까 훅훅
// 지나가고". 임펄스 순간의 스로틀만으로는 멀리서 실은 운동량이 근접에서도
// 광속이다 — 감속은 상태가 아니라 **매 프레임의 법**이어야 한다.
/** 조준 유지 반경(px). 실제 손은 휠을 굴리는 동안 커서가 수 px 흔들린다 —
 *  8px 은 도구의 손이지 사람의 손이 아니었다(관찰자 0번 노트, 문 0 3차 실증).
 *  이 반경 안의 흔들림은 같은 뜻이고, 그 밖으로 옮겨 굴리면 새 뜻이다. */
export const AIM_HOLD_PX = 24;
/** 조속기 — 공간은 천체 곁에서 끈적해진다. 유효 속도는 매 프레임
 *  GOV_FLOOR + GOV_GAIN × (남은 거리 − ARRIVE_STANDOFF) 에 묶인다. 남은
 *  거리는 뜻한 별까지, 뜻이 없으면 최근접 별까지다. 1/GOV_GAIN ≈ 접근의
 *  시간상수(초): 남은 거리가 매초 e 분의 1 로 준다 — 브레이크를 밟는 것이
 *  아니라 공간이 밟아 준다. */
export const GOV_GAIN = 3;
/** 조속기 바닥 속도 — 표면 곁에서도 배는 걸을 수 있다(0 이면 갇힌다) */
export const GOV_FLOOR = 60;
/** 고개 응답 — 고르는 것은 몸을 옮기지 않지만(R13-c 불변), 화면 밖의 별을
 *  카드에서 지목하면 고개가 그쪽으로 돈다. 카드만 바뀌고 하늘이 침묵하면
 *  유령 카드다(문 0 3차: 조이스 카드 아래 빈 하늘). 최대 각속도 rad/s. */
export const PICK_TURN_RATE = 2.4;
/** 고개 응답의 문턱 — 별이 **실제 투영에서** 화면 밖일 때만 돈다(NDC 여유
 *  1.02). 화면 안의 별은 커서 자리가 곧 별 자리라 돌지 않는다. 각도로 재면
 *  모서리 별(시야 대각 ≈0.66rad)이 세로 시야 문턱을 넘겨 오발한다(계약
 *  실측: 하늘이 196px 밀렸다). */
export const PICK_TURN_NDC = 1.02;
