...
| `AGENTS.md` (imported by `CLAUDE.md`) | Persistent **operational** instructions for coding agents; owns the hard constraints and data invariants directly. | Authoritative for agent behavior/workflow, and for the constraints it states itself; points to the canonical docs for policy detail rather than restating it. | active |
| `.claude/rules/*` | Action cards loaded when matching files are opened. | **No authority of its own** — carries an explicit non-authority declaration; if a card and the canonical docs diverge, the card is wrong. | derived |
...
| `foundry/decisions/*.json` | Committed batch decision files (audit trail, §15.2). | The re-audit record; replay-bearing. | frozen **as to replay input** — adds, promotions, verdicts, identity records and sanctions are what a replay consumes and are never edited in place. Three carve-outs, all narrow: (1) the **provenance** field `qc_by` and the generated `.report.md` may be corrected when the recorded value is measurably false, with the original preserved verbatim in an appended `★ PROVENANCE CORRECTION` note; (2) **tool-managed anchor fields** — `snapshot_url` / `revision_permalink` on a verdict source, and `anchors_pending` — are written and rewritten by `foundry:anchor` by design, which is how evidence permanence is maintained over time; (3) `notes` are append-only. |
