...
| `foundry/decisions/*.json` | Committed batch decision files (audit trail, §15.2). | The re-audit record; replay-bearing. | frozen, with one carve-out: a **provenance** field (`qc_by`) and the generated `.report.md` may be corrected in place when the recorded value is measurably false, and the original value must be preserved verbatim in an appended `★ PROVENANCE CORRECTION` note. Nothing else is edited in place — adds, promotions, verdicts, identity records and sanctions are the replay input and are append-only. |
